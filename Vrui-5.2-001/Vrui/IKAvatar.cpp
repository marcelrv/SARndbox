...
void setPosition(SceneGraph::TransformNode& node,const Point& position) // Sets a transform node's translation based on an origin position
	{
	/* Set the transform node's translation: */
	node.translation.setValue(SceneGraph::Vector(position-Point::origin));
	node.update();
	}
...
void IKAvatar::configureAvatar(const Configuration& configuration)
	...
	setPosition(*neckNode,Point::origin);
	
	for(int armIndex=0;armIndex<2;++armIndex)
		{
		setPosition(*arms[armIndex].clavicleNode,configuration.arms[armIndex].claviclePos);
		setPosition(*arms[armIndex].shoulderNode,configuration.arms[armIndex].shoulderPos);
		setPosition(*arms[armIndex].elbowNode,Point(0,configuration.arms[armIndex].upperLength,0));
		setPosition(*arms[armIndex].wristNode,Point(0,configuration.arms[armIndex].lowerLength,0));
		}
	
	setPosition(*pelvisNode,configuration.pelvisPos);
	
	for(int legIndex=0;legIndex<2;++legIndex)
		{
		setPosition(*legs[legIndex].hipNode,configuration.legs[legIndex].hipPos);
		setPosition(*legs[legIndex].kneeNode,Point(0,0,-configuration.legs[legIndex].upperLength));
		setPosition(*legs[legIndex].ankleNode,Point(0,0,-configuration.legs[legIndex].lowerLength));
		}
