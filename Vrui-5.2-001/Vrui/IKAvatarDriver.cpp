...
void IKAvatarDriver::configure(const char* configName)
	{
	/* Open the base IKAvatar configuration file section: */
	Misc::ConfigurationFileSection baseCfg=getModuleConfigurationSection("IKAvatar");
	
	/* Open the requested configuration file section: */
	Misc::ConfigurationFileSection cfg;
	if(configName!=0)
		cfg=baseCfg.getSection(configName);
	else
		{
		/* Find the first configuration sub-section: */
		cfg=Misc::ConfigurationFileSection(baseCfg.beginSubsections());
		}
	
	/* Bail out if the configuration file section does not exist: */
	if(!cfg.isValid())
		{
		if(configName!=0)
			Misc::formattedUserError("Vrui::IKAvatar::Configure: Avatar configuration section %s does not exist",configName);
		else
			Misc::userError("Vrui::IKAvatar::Configure: No default avatar configuration section");
...
void IKAvatarDriver::configureAvatar(IKAvatar& avatar) const
	{
	/* Load the avatar model file: */
	avatar.loadAvatar(avatarModelFileName.c_str());
	
	/* Configure the avatar: */
	avatar.configureAvatar(getConfiguration());
	}
...
inline Vector calcForwardDirectionAndPitch(const Rotation& r,Scalar& pitchAngle) // Calculates a forward-facing vector and a pitch angle for a rotation that might look straight up or down or slightly backwards
	...
	/* Calculate a forward direction by blending the two vectors weighted by their length: */
	Vector forward(hx*hxLen+hy*hyLen);
	/* ^^^ Weighing isn't really necessary. Investigate! */
	
	/* Calculate the pitch angle: */
	pitchAngle=safeAcos((y*forward)/Geometry::mag(forward));
	if(y[2]<Scalar(0))
...
bool IKAvatarDriver::calculateState(IKAvatar::State& state)
	...
	*********************************************************************/
	...
	/* Initialize the pelvis transform: */
	ONTransform pelvis=ONTransform::translateFromOriginTo(pelvisPos);
	...
	*********************************************************************/
	...
	for(int legIndex=0;legIndex<2;++legIndex)
		...
		if(htaLen2<leg.length2)
			...
			Scalar alpha=Math::acos((htaLen2+leg.upperLength2-leg.lowerLength2)/(Scalar(2)*Math::sqrt(htaLen2)*leg.upperLength));
			sLeg.hip*=Rotation::rotateX(alpha);
			
			/* Rotate the hip slightly outwards: */
			sLeg.hip.leftMultiply(Rotation::rotateAxis(hipToAnkle,Math::rad(legIndex==0?Scalar(-7.5):Scalar(7.5))));
	...
	if(legs[0].planted&&legs[1].planted)
		{
		Vector pd=ikPelvis.getDirection(1);
		pd/=Math::sqrt(Math::sqr(pd[0])+Math::sqr(pd[1]));
		Scalar lcos=fldir[0]*pd[0]+fldir[1]*pd[1];
		Scalar rcos=frdir[0]*pd[0]+frdir[1]*pd[1];
