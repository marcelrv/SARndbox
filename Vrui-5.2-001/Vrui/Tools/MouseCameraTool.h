...
class MouseCameraTool:public UtilityTool
	...
	/* States of controlled screen and viewer when the tool was created: */
	...
	Vector base[3]; // Base vectors of environment (right, forward, up)
	InputDevice* screenDevice; // Device to which the controlled screen was attached
	ONTransform screenDeviceTransform; // Initial screen transformation in device state if screen was device-attached
	ONTransform screenTransform; // Initial physical-space screen transformation when tool was created
	Scalar screenSize[2]; // Initial size of screen when tool was created
	Scalar screenDiagonal; // Original diagonal size of screen to scale interactions
	InputDevice* viewerDevice; // Device from which the viewer received tracking data
	TrackerState viewerTransform; // Initial physical-space viewer head transformation when tool was created
	Point viewerEyes[3]; // Positions of viewer's eyes (mono, left, right) in viewer space when tool was created
	Vector viewDirection; // Initial vector from viewer's mono eye position to screen's center in physical space
	ONTransform normTransform; // Transformation that normalizes the initial virtual camera state, i.e., brings the screen center to the origin and aligns the viewing direction with the forward direction
