...
bool vruiHandleAllEvents(bool allowBlocking)
	...
	if(readFdSet.isSet(fileno(stdin)))
		...
		if(read(fileno(stdin),readBuffer,sizeof(readBuffer))>0)
			{
			if(readBuffer[0]==27)
				{
	...
	if(readFdSet.isSet(fileno(stdin)))
		{
		vruiState->commandDispatcher.dispatchCommands(fileno(stdin));
		handledEvents=true;
		}
	...
	if(vruiCommandPipe>=0&&readFdSet.isSet(vruiCommandPipe))
		{
		vruiState->commandDispatcher.dispatchCommands(vruiCommandPipe);
		handledEvents=true;
		}
...
void mainLoop(void)
	...
	#if 0
	/* Check if this is a windowless cluster head node: */
	if(vruiState->master&&vruiNumWindows==0)
		{
		/* Listen for events on stdin to shut down Vrui: */
