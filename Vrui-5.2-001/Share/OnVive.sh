...
VRUI_OPTIONS="$VRUI_OPTIONS -setConfig $VRUIWINSEC/outputName=$VIVE_PORT"
VRUI_OPTIONS="$VRUI_OPTIONS -setConfig $VRUIWINSEC/LensCorrector/superSampling=$SUPERSAMPLING"
if [ $USE_CONTROLWINDOW -ne 0 ]; then
	VRUI_OPTIONS="$VRUI_OPTIONS -mergeConfig ControlWindow"
fi
if [ $SHOW_CONTROLLERS -ne 0 ]; then
	VRUI_OPTIONS="$VRUI_OPTIONS -vislet DeviceRenderer ;"
fi
